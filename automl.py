...
if __name__ == '__main__':
    ...
    #os.chdir("/root/automlbasketball")
    #if ((hour != 0) & (hour != 11)):
    root = os.getcwd()  # 获取当前路径
    state = workif.workif('read', 0)
    print(state)
    if(state == 0):
        workif.workif('write', 1)
        time_start = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())

        gameid = automl_spider.spider("NBA")
        new_waitid = organize.organize(gameid,"NBA")
        for i in new_waitid:
            automl_spider.basketballspider(i)
        forecast.forecast("NBA")
        forecast.forecast("CBA")

        gameid = automl_spider.spider("FOOTBALL")
        new_waitid = organize.organize(gameid, "FOOTBALL")
        for i in new_waitid:
            automl_spider.footballspider(i)
        forecast.forecast("FOOTBALL")

        if ((hour == 13) & (min <= 15) & (min >= 0)):
            predfit.pred_fit("NBA")
            organize.delay_clear("NBA")
        if ((hour == 21) & (min <= 15) & (min >= 0)):
            predfit.pred_fit("CBA")
            organize.delay_clear("CBA")
        if ((hour == 11) & (min <= 15) & (min >= 0)):
            predfit.pred_fit("FOOTBALL")
            organize.delay_clear("FOOTBALL")
        time_end = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        workif.workif('write', 0)
        print(time_start)
        print(time_end)
...
#     print(result)
#
#     result_df = pd.DataFrame(result)#, columns=['编号', '时间', '联赛', '主队', '客队', '联赛', '公司1', '初主', '初盘', '初客', '终主', '终盘', 
#'终客', '公司2', '初主', '初盘', '初客', '终主', '终盘', '终客', '公司3', '初主', '初盘', '初客', '终主', '终盘', '终客', '公司4', '初主', '初盘', '初客', '终主', '终盘', '终客', '公司5', '初主', 
#'初盘', '初客', '终主', '终盘', '终客', '公司6', '初主', '初盘', '初客', '终主', '终盘', '终客'])
