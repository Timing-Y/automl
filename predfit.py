...
def pred_fit(Lname):
    ...
    if Lname == 'NBA':
        time_now = time.strftime("%Y-%m-%d 14:00", time.localtime())
        ...
        yesterday = now - 86400
        time_yesterday = time.strftime("%Y-%m-%d 14:00", time.localtime(yesterday))
    elif Lname == 'CBA':
        time_now = time.strftime("%Y-%m-%d 22:00", time.localtime())
        if (time.strftime("%Y-%m-%d %H:%M", time.localtime()) > time_now):
            now = int(time.mktime(time.strptime(str(time_now), "%Y-%m-%d %H:%M")))
        else:
            now = int(time.mktime(time.strptime(str(time_now), "%Y-%m-%d %H:%M"))) - 86400
        yesterday = now - 86400
        time_yesterday = time.strftime("%Y-%m-%d 22:00", time.localtime(yesterday))
    elif Lname == 'FOOTBALL':
    ...
    if (week != 1):
        ...
        train_data_w = train_data.loc[train_data['week'] == (week)].copy()
    if (month != 1):
        train_data_m = train_data.loc[train_data['month'] == (month - 1)].copy()
    else:
